...
func ValidateUserSettable(id string) error {
	if len(id) < 4 || 63 < len(id) {
		return status.Errorf(
			codes.InvalidArgument, "user-settable ID must be between 4 and 63 characters")
	}
	if !unicode.IsLetter(rune(id[0])) {
		return status.Errorf(
			codes.InvalidArgument, "user-settable ID must begin with a letter")
	}
	if id[len(id)-1] == '-' {
		return status.Errorf(
			codes.InvalidArgument, "user-settable ID must end with a letter or number")
	}
	if _, err := uuid.Parse(id); err == nil {
		return status.Errorf(
			codes.InvalidArgument, "user-settable ID must not be a valid UUIDv4")
	}
