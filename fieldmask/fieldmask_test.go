...
import (
	...
	"github.com/sraphs/aip/internal/testdata/syntax/v1"
)
		...
		for _, tt := range []struct {
			...
			},
			{
				...
				src: &syntax.Message{
					...
					MapStringMessage: map[string]*syntax.Message{
						"src-key": {
							String_: "src-value",
						},
					},
			...
			},
			{
				name: "oneof: swap",
				src: &syntax.Message{
					Oneof: &syntax.Message_OneofString{
						OneofString: "src",
					},
				},
				dst: &syntax.Message{
					Oneof: &syntax.Message_OneofMessage2{
						OneofMessage2: &syntax.Message{
							String_: "dst",
						},
					},
				},
			},
			{
				...
				src: &syntax.Message{
					Oneof: &syntax.Message_OneofMessage1{
						OneofMessage1: &syntax.Message{
							String_: "src",
						},
					},
				},
	...
	t.Run("wire set fields", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			},
			{
				name: "nested",
				src: &syntax.Message{
					Message: &syntax.Message{String_: "src"},
				},
				dst: &syntax.Message{
					Message: &syntax.Message{
						String_: "dst",
			...
			},
			{
				...
				src: &syntax.Message{
					Oneof: &syntax.Message_OneofMessage1{
						OneofMessage1: &syntax.Message{String_: "src"},
					},
				},
				dst: &syntax.Message{
					Oneof: &syntax.Message_OneofMessage1{
	...
	t.Run("paths", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			},
			{
				...
				src: &syntax.Message{
					RepeatedMessage: []*syntax.Message{
						{String_: "src"},
						{Int64: 111},
					},
				},
