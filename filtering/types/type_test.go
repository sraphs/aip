...
func TestTypeDescriptionFieldMap(t *testing.T) {
	pbdb := NewDb()
	msg := &testdata.NestedTestAllTypes{}
...
func TestFieldDescriptionGetFrom(t *testing.T) {
	...
	msg := &testdata.TestAllTypes{
		...
		NestedType: &testdata.TestAllTypes_SingleNestedMessage{
			SingleNestedMessage: &testdata.TestAllTypes_NestedMessage{
				Bb: 123,
			},
		},
	...
	expected := map[string]interface{}{
		...
		"single_nested_message": &testdata.TestAllTypes_NestedMessage{
			Bb: 123,
		},
...
func TestFieldDescriptionIsSet(t *testing.T) {
	...
	tests := []struct {
		...
		{
			msg:   &testdata.TestAllTypes{SingleBool: true},
			field: "single_bool",
			isSet: true,
		},
		{
		...
		{
			msg:   (&testdata.TestAllTypes{SingleBool: false}),
			field: "single_bool",
			isSet: false,
		},
		{
		...
		{
			msg:   reflect.ValueOf(&testdata.TestAllTypes{}),
			field: "single_bool",
			isSet: false,
		},
		{
...
func TestTypeDescriptionMaybeUnwrap(t *testing.T) {
	pbdb := NewDb()
	_, err := pbdb.RegisterMessage(&testdata.TestAllTypes{})
	if err != nil {
	...
	tests := []struct {
		...
		{
			in:  dynMsg(t, anyMsg(t, &testdata.TestAllTypes{SingleFloat: 123.0})),
			out: &testdata.TestAllTypes{SingleFloat: 123.0},
		},
		{
...
func BenchmarkTypeDescriptionMaybeUnwrap(b *testing.B) {
	pbdb := NewDb()
	pbdb.RegisterMessage(&testdata.TestAllTypes{})
	msgType := "google.protobuf.Value"
	msgDesc, found := pbdb.DescribeType(msgType)
	if !found {
